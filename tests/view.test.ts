...
const ESTIMATE_DOC = {
  ...
  estimate: {
    object: "estimate",
    data: { estimate_number: "1001", estimate_uuid: "est_x" },
  },
  options: {
    object: "list",
    data: [
      {
        ...
        line_items: {
          object: "list",
          data: [{ name: "Flush", unit_price: 32000, amount: 32000 }],
        },
      },
    ],
  },
...
async function harness(doc: unknown, token = ESTIMATE_TOKEN) {
  ...
  const client = new HousecallProClient(
    new LinkRegistry({ HOUSECALLPRO_LINK: token }),
    {
...
describe("the rung ladder", () => {
  ...
  it("defaults to compact when no view is named", async () => {
    ...
    expect(
      (out["options"] as Record<string, unknown>[])[0]?.["total_amount_usd"],
    ).toBe(346.39);
  });
  ...
  it("advertises the rungs, and their money difference, in the parameter description", async () => {
    ...
    const estimate = tools.tools.find(
      (t) => t.name === "housecallpro_get_estimate",
    );
    const view = (
      estimate?.inputSchema.properties as Record<
        string,
        { description?: string }
      >
    )?.["view"];
...
describe("what must never happen", () => {
  ...
  it("minifies compact and indents raw", async () => {
    const { h } = await harness(ESTIMATE_DOC);
    expect(
      textOf(await h.callTool("housecallpro_get_estimate", {})),
    ).not.toMatch(/\n/);
    expect(
      textOf(await h.callTool("housecallpro_get_estimate", { view: "raw" })),
    ).toMatch(/\n/);
  });
});
...
describe("a projection that loses its footing", () => {
  it("hands back the whole document instead of an empty summary", () => {
    ...
    expect(warn).toHaveBeenCalledWith(
      expect.stringMatching(/could not project/),
    );
    warn.mockRestore();
  });
