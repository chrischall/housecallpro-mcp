 ...
 *
 * The read tools in this server hand back Housecall Pro's payload close to
 ...
 * honestly say which of Housecall Pro's fields matter and which are noise.
 *
 ...
 */
export const HCP_VIEWS = ['compact', 'full'] as const;

const NOTE =
  'compact strips image/avatar URLs from the response; "full" returns Housecall Pro\'s payload untouched. ' +
  'No field projection: this server has no verified record of which Housecall Pro fields matter, and inventing ' +
  'one would risk dropping a field a caller needs.';

/** The `view` parameter every read tool in this server takes. */
export const viewArg = (): ReturnType<typeof viewParam> => viewParam(HCP_VIEWS, { note: NOTE });
...
export function viewResponse(view: string | undefined, data: unknown): ReturnType<typeof minifiedResult> {
  const rung: View = resolveView(view, HCP_VIEWS);
